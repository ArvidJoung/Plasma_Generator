...
/*-----------------------------------------------------------*/
static void waitInEvntCompletionSignal(QueueSetMemberHandle_t xQueue)
{
	...
	for(;;)
	{
		...
		if( (cmd.cmd == OUTEVENT_INIT_COMPLETE_EVENT) || (cmd.cmd == 0) )
		{
...
/*-----------------------------------------------------------*/
void mctask_init(QueueSetMemberHandle_t  xQueue)
{
	mcCLIregister();
	...
	waitInEvntCompletionSignal(xQueue);

	//Initialize Plasma Level & Vol value to 0
...
static void vMCTask( void *pvParameters )
{
	...
	for(;;)
	{
		...
		switch(cmd.cmd)
		{
            ...
            case MC_DOWN_KEY_PRESSED_F:
            case MC_UP_KEY_PRESSED_F:
                PlasmaVolSet(cmd.cmd);
                break;

            case MC_GND_JACK_IN_EVT:
                break;
                
            case MC_GND_JACK_OUT_EVT:
                break;
