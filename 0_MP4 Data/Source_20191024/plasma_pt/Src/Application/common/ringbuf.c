...
int
ringbuf_getavg(const struct ringbuf_t *rb)
{
    uint8_t *startPtr;
    uint8_t count,length = ringbuf_bytes_used(rb);
    int sum=0,avg=0;
    
    if( (ringbuf_is_full(rb)) || (rb->head >= rb->tail))
    {
        count = length;
    ...
    {
        count = rb->head - rb->buf;
        startPtr = rb->buf;
                    
        sum = ringsum(startPtr,count);
        count = ringbuf_end(rb) - rb->tail;  
        startPtr = rb->tail;
    }
...
int
ringbuf_add_getAvg(ringbuf_t dst, int data)
{
    const uint8_t *bufend = ringbuf_end(dst);
    int overflow = 1 > ringbuf_bytes_free(dst);

    // add data to head buff
    *dst->head = data;
    ...
    return ringbuf_getavg(dst);
}

int
ringmax(uint8_t *buf, int count, int max)
{
    int i;
...
int
ringbuf_getmax(const struct ringbuf_t *rb)
{
    uint8_t *startPtr;
    uint8_t count,length = ringbuf_bytes_used(rb);
    int max=0;
    ...
    else
    {
        ...
        max = ringmax(startPtr,count,max);
        count = ringbuf_end(rb) - rb->tail;  
        startPtr = rb->tail;
    }
...
int
ringbuf_add_getMax(ringbuf_t dst, int data)
{
    const uint8_t *bufend = ringbuf_end(dst);
    int overflow = 1 > ringbuf_bytes_free(dst);
